...
class H5Resolver:
    ...
    def __init__(self, scan_group):
        # scan_group is /Sxxxxx for legacy HDF5 files and /entry0 for Nomad NeXus files.
        self.SCAN = scan_group
        self.INST = scan_group.get("instrument") if "instrument" in scan_group else None
        try:
            self.FILE_PATH = os.path.abspath(scan_group.file.filename)
        except Exception:
            self.FILE_PATH = ""
...
# --- BEGIN: Nomad NeXus (.nxs) support for SuperADAM ---
class _H5InterleavedDetectorView:
    """Lightweight view of an HDF5 detector stack with optional spin splitting.

    Nomad currently stores non-spin-flip data in one detector dataset as
    up0, down0, up1, down1, ... .  The detector image axes in that file are
    (x, y), while pySAred expects (y, x).  This wrapper maps stack[i] to the
    requested spin frame and transposes detector axes on the fly without
    loading the whole detector stack into memory.
    """

    def __init__(self, dataset, offset=0, stride=2, transpose_yx=False):
        self._dataset = dataset
        self._offset = int(offset)
        self._stride = int(stride)
        self._transpose_yx = bool(transpose_yx)
        self._n = len(range(self._offset, int(dataset.shape[0]), self._stride))
        if self._transpose_yx and len(dataset.shape) >= 3:
            self.shape = (self._n, int(dataset.shape[2]), int(dataset.shape[1])) + tuple(dataset.shape[3:])
        else:
            self.shape = (self._n,) + tuple(dataset.shape[1:])
        self.ndim = len(self.shape)
        self.dtype = dataset.dtype

    def _map_first_index(self, idx):
        if isinstance(idx, (int, _np.integer)):
            i = int(idx)
            if i < 0:
                i += self._n
            return self._offset + self._stride * i
        if isinstance(idx, slice):
            start, stop, step = idx.indices(self._n)
            return slice(self._offset + self._stride * start,
                         self._offset + self._stride * stop,
                         self._stride * step)
        arr = _np.asarray(idx)
        return self._offset + self._stride * arr

    def _transpose_result(self, arr, first_index_scalar):
        if not self._transpose_yx:
            return arr
        arr = _np.asarray(arr)
        if first_index_scalar:
            if arr.ndim >= 2:
                return _np.swapaxes(arr, 0, 1)
            return arr
        if arr.ndim >= 3:
            return _np.swapaxes(arr, 1, 2)
        return arr

    def __getitem__(self, key):
        if not isinstance(key, tuple):
            mapped_first = self._map_first_index(key)
            arr = self._dataset[mapped_first]
            return self._transpose_result(arr, isinstance(key, (int, _np.integer)))

        if len(key) == 0:
            return self._dataset[key]

        first_key = key[0]
        mapped_first = self._map_first_index(first_key)
        first_scalar = isinstance(first_key, (int, _np.integer))

        if not self._transpose_yx or len(key) == 1:
            arr = self._dataset[(mapped_first,) + tuple(key[1:])]
            return self._transpose_result(arr, first_scalar)

        # pySAred keys after the frame index are in (y, x) order.  Nomad raw
        # detector frames are in (x, y), so swap those two selectors before
        # reading and then transpose the resulting array back for pySAred.
        y_sel = key[1] if len(key) > 1 else slice(None)
        x_sel = key[2] if len(key) > 2 else slice(None)
        rest = tuple(key[3:])
        raw_key = (mapped_first, x_sel, y_sel) + rest
        arr = self._dataset[raw_key]
        return self._transpose_result(arr, first_scalar)

    def __array__(self, dtype=None):
        arr = self._dataset[self._offset:self._offset + self._stride * self._n:self._stride, ...]
        arr = self._transpose_result(arr, first_index_scalar=False)
        if dtype is not None:
            arr = arr.astype(dtype, copy=False)
        return arr


def _h5_scalar_value(ds, default=None):
    try:
...
def _h5_text_value(value):
    try:
        ...
        if isinstance(value, (_np.bytes_, bytes)):
...
def _h5_1d_numeric(ds):
    if not isinstance(ds, _h5py.Dataset):
        return None
    try:
        return _np.asarray(ds[()], dtype=float).reshape(-1)
    except Exception:
        return None


def _nomad_norm_label(label):
    """Return a compact, case-insensitive key for Nomad scanned-variable labels."""
    txt = _h5_text_value(label).strip().lower()
    return "".join(ch for ch in txt if ch.isalnum())


def _nomad_label_keys(label):
    txt = _h5_text_value(label).strip()
    low = txt.lower()
    variants = (txt, low, low.replace(" ", "_"), low.replace("-", "_"), _nomad_norm_label(txt))
    keys = []
    for key in variants:
...
_NOMAD_TH_ALIASES = (
    "th", "theta", "sample_theta", "sample theta", "sampletheta",
    "samth", "th_sam", "th_sample",
)
_NOMAD_TTH_ALIASES = (
    "tth", "2th", "two_theta", "two theta", "twotheta", "a2",
)
_NOMAD_MONITOR_ALIASES = (
    # New Nomad/SuperADAM interface writes the real monitor as Monitor1.
    "Monitor1", "monitor1", "monitor_1", "monitor 1", "mon1", "m1",
    # Keep older/generic names for compatibility with intermediate files.
    "monitor", "mon", "mon0", "monitorcount", "monitor_count", "monitor count",
)
_NOMAD_TOTALSUM_ALIASES = (
    "totalsum", "total_sum", "total sum", "TotalSum",
    "TotalCount", "totalcount", "total_count", "total count",
    "detsum", "det_sum", "detector_sum", "detector sum",
)
_NOMAD_ROISUM_ALIASES = (
    "roisum", "roi_sum", "roi sum", "roi", "roi1", "roi_1",
)
_NOMAD_TIME_ALIASES = (
    "Time", "time", "sec", "secs", "second", "seconds",
    "duration", "preset", "counttime", "count_time", "count time",
)

# User-configurable Nomad detector/scanned-variable unpacking.
# The labels are the names pySAred uses internally and in exported file names.
# For 2-state NSF, uu means I+ (= uu + ud) and dd means I- (= dd + du).
_NOMAD_DEFAULT_UNPACK_ORDERS = {
    "none": ["uu"],
    "nsf_2": ["uu", "dd"],
    "pnr_4": ["uu", "ud", "dd", "du"],
}
_NOMAD_MODE_LABELS = {
    "none": "No unpacking / NR",
    "nsf_2": "2-state NSF unpacking",
    "pnr_4": "4-state PNR unpacking",
}
_NOMAD_ALLOWED_COMPONENT_LABELS = ("uu", "ud", "du", "dd")
_NOMAD_UNPACK_CONFIG_BY_FILE = {}


def _nomad_file_key_from_path(path):
    try:
...
def _nomad_file_key(self):
    try:
        if getattr(self, "FILE_PATH", ""):
            return _nomad_file_key_from_path(self.FILE_PATH)
        return _nomad_file_key_from_path(self.SCAN.file.filename)
    except Exception:
        return ""


def _nomad_mode_from_n_components(n_components):
    try:
...
def _nomad_default_order_for_mode(mode):
    return list(_NOMAD_DEFAULT_UNPACK_ORDERS.get(str(mode), _NOMAD_DEFAULT_UNPACK_ORDERS["none"]))


def _nomad_normalize_unpack_config(config, detected_components=None, detected_reason=""):
    config = dict(config or {})
    mode = str(config.get("mode", "auto")).strip().lower()
    if mode == "auto":
        mode = _nomad_mode_from_n_components(detected_components or 1)
    if mode not in _NOMAD_DEFAULT_UNPACK_ORDERS:
        mode = "none"

    expected_n = len(_NOMAD_DEFAULT_UNPACK_ORDERS[mode])
    order = config.get("order", None)
    if order is None:
        order = _nomad_default_order_for_mode(mode)
    else:
        order = [str(x).strip().lower() for x in order if str(x).strip()]

    valid = []
    for label in order:
        if label in _NOMAD_ALLOWED_COMPONENT_LABELS and label not in valid:
            valid.append(label)
    if len(valid) != expected_n:
        valid = _nomad_default_order_for_mode(mode)

    return {
        "mode": mode,
        "mode_label": _NOMAD_MODE_LABELS.get(mode, mode),
        "order": valid,
        "n_components": len(valid),
        "source": str(config.get("source", "auto")),
        "user_confirmed": bool(config.get("user_confirmed", False)),
        "detected_components": int(detected_components or config.get("detected_components", len(valid)) or len(valid)),
        "detected_reason": str(detected_reason or config.get("detected_reason", "")),
    }


def _nomad_get_unpack_config_for_path(path):
    return _NOMAD_UNPACK_CONFIG_BY_FILE.get(_nomad_file_key_from_path(path))


def _nomad_set_unpack_config_for_path(path, config):
    key = _nomad_file_key_from_path(path)
    cfg = _nomad_normalize_unpack_config(config,
                                         config.get("detected_components") if isinstance(config, dict) else None,
                                         config.get("detected_reason", "") if isinstance(config, dict) else "")
    cfg["source"] = str(config.get("source", "user") if isinstance(config, dict) else "user")
    cfg["user_confirmed"] = bool(config.get("user_confirmed", True) if isinstance(config, dict) else True)
    _NOMAD_UNPACK_CONFIG_BY_FILE[key] = cfg
    return cfg


def _nomad_is_nomad(self):
    try:
        return (
            isinstance(self.SCAN, _h5py.Group)
            and "data_scan" in self.SCAN
            and "SUPERADAM" in self.SCAN
            and isinstance(self.SCAN.get("data_scan/detector_data/data"), _h5py.Dataset)
        )
    except Exception:
        return False


def _nomad_dataset(self, rel_path):
    try:
        return self.SCAN.get(rel_path)
    except Exception:
        return None


def _nomad_detector_raw(self):
    return _nomad_dataset(self, "data_scan/detector_data/data")


def _nomad_scanned_table(self):
    data = _nomad_dataset(self, "data_scan/scanned_variables/data")
    labels = _nomad_dataset(self, "data_scan/scanned_variables/variables_names/label")
    if not (isinstance(data, _h5py.Dataset) and isinstance(labels, _h5py.Dataset)):
        return {}, []

    try:
        raw_names = _np.asarray(labels[()]).reshape(-1)
        names = [_h5_text_value(x).strip() for x in raw_names]
    ...
    # Nomad normally writes (nVariables, nFrames), but accept common variants:
    # (nFrames, nVariables), scalar/constant vectors, or an extra singleton dim.
    arr = _np.squeeze(arr)
    if arr.ndim == 0:
        arr = arr.reshape(1, 1)
    elif arr.ndim == 1:
        if n_names == 1:
            arr = arr.reshape(1, -1)
        elif arr.size == n_names:
            arr = arr.reshape(n_names, 1)
    elif arr.ndim == 2:
        if arr.shape[0] == n_names:
            pass
        elif arr.shape[1] == n_names:
            arr = arr.T
    else:
        matching_axes = [axis for axis, size in enumerate(arr.shape) if size == n_names]
        if matching_axes:
            arr = _np.moveaxis(arr, matching_axes[0], 0).reshape(n_names, -1)
        else:
            arr = arr.reshape(arr.shape[0], -1)

    out = {}
    if arr.ndim == 2 and arr.shape[0] == n_names:
        for name, vec in zip(names, arr):
            vec = _np.asarray(vec)
            # Register both the original/case-folded label and a compact key so
            # Monitor1, monitor_1, and "Monitor 1" resolve identically.
            for key in _nomad_label_keys(name):
                out.setdefault(key, vec)
    return out, names


def _nomad_total_steps(self):
    """Return the nominal number of angular points recorded by Nomad, if present."""
    for rel in ("data_scan/total_steps", "data_scan/actual_step"):
        v = _h5_scalar_value(_nomad_dataset(self, rel), None)
        if v is not None:
            try:
                iv = int(v)
                if iv > 0:
...
def _nomad_raw_variable_from_table(self, aliases):
    table, _labels = _nomad_scanned_table(self)
    for label in aliases:
        for key in _nomad_label_keys(label):
            vec = table.get(key)
            if vec is not None:
                return _np.asarray(vec).reshape(-1), label
    return None, None


def _nomad_values_repeat_in_groups(vec, group_size, nframes):
    try:
        arr = _np.asarray(vec, dtype=float).reshape(-1)
        group_size = int(group_size)
        if group_size <= 1 or len(arr) != int(nframes) or int(nframes) % group_size:
            return False
        arr = arr.reshape(int(nframes) // group_size, group_size)
        ref = arr[:, [0]]
        return bool(_np.allclose(arr, ref, equal_nan=True))
    except Exception:
        return False


def _nomad_detect_component_count(self):
    """Infer whether the raw Nomad frame stack looks like NR, 2-state NSF, or 4-state PNR."""
    raw = _nomad_detector_raw(self)
    if not isinstance(raw, _h5py.Dataset) or raw.ndim < 3:
        return 1, "No 3D detector stack was found."

    nframes = int(raw.shape[0])
    if nframes <= 1:
        return 1, "The detector stack has one frame."

    total_steps = _nomad_total_steps(self)
    if total_steps > 0:
        if nframes == total_steps:
            return 1, f"Detector frames ({nframes}) equal Nomad total_steps ({total_steps})."
        for n_components in (4, 2):
            if nframes == n_components * total_steps:
                return n_components, (
                    f"Detector frames ({nframes}) equal {n_components} x Nomad total_steps ({total_steps})."
                )

    # If total_steps is missing or inconclusive, use repeated th/tth values.
    for n_components in (4, 2):
        if nframes % n_components:
            continue
        for aliases in (_NOMAD_TH_ALIASES, _NOMAD_TTH_ALIASES):
            vec, label = _nomad_raw_variable_from_table(self, aliases)
            if vec is None:
                continue
            if _nomad_values_repeat_in_groups(vec, n_components, nframes):
                return n_components, (
                    f"Scanned variable '{label}' repeats in groups of {n_components} frames."
                )

    return 1, "No reliable repeated-frame pattern was detected."


def _nomad_unpack_config(self):
    detected_components, detected_reason = _nomad_detect_component_count(self)
    key = _nomad_file_key(self)
    stored = _NOMAD_UNPACK_CONFIG_BY_FILE.get(key)
    if stored is not None:
        cfg = dict(stored)
        cfg["detected_components"] = detected_components
        cfg["detected_reason"] = detected_reason
        return _nomad_normalize_unpack_config(cfg, detected_components, detected_reason)

    # Safe non-interactive default: use the detected mode.  GUI imports replace
    # this with a user-confirmed configuration before reduction/preview.
    return _nomad_normalize_unpack_config({
        "mode": _nomad_mode_from_n_components(detected_components),
        "source": "auto",
        "user_confirmed": False,
    }, detected_components, detected_reason)


def _nomad_component_order(self):
    return list(_nomad_unpack_config(self).get("order", ["uu"]))


def _nomad_component_count(self):
    return len(_nomad_component_order(self))


def _nomad_has_interleaved_spin_pairs(self):
    return _nomad_component_count(self) == 2


def _nomad_pol_offset(self, pol=None):
    order = _nomad_component_order(self)
    p = str(pol or order[0] if order else "uu").strip().lower()
    aliases = {
        "up": "uu", "spin_up": "uu", "spin-up": "uu", "spin up": "uu", "iplus": "uu", "i+": "uu",
        "down": "dd", "spin_down": "dd", "spin-down": "dd", "spin down": "dd", "iminus": "dd", "i-": "dd",
    }
    p = aliases.get(p, p)
    try:
...
def _nomad_npoints(self, pol=None):
    raw = _nomad_detector_raw(self)
    if isinstance(raw, _h5py.Dataset):
        nframes = int(raw.shape[0])
        n_components = _nomad_component_count(self)
        if n_components > 1:
            return len(range(_nomad_pol_offset(self, pol), nframes, n_components))
        return nframes
    steps = _nomad_total_steps(self)
    return int(steps) if steps else 0


def _nomad_prepare_variable(self, vec, pol=None):
    vec = _np.asarray(vec)
    if vec.ndim != 1:
        vec = vec.reshape(-1)
    ...
    raw = _nomad_detector_raw(self)
    n_components = _nomad_component_count(self)
    if n_components > 1 and isinstance(raw, _h5py.Dataset) and len(vec) == int(raw.shape[0]):
        return vec[_nomad_pol_offset(self, pol)::n_components]

    n = _nomad_npoints(self, pol)
    if n and len(vec) == 1:
        return _np.full(n, vec[0])
    return vec


def _nomad_unpack_summary(self):
    cfg = _nomad_unpack_config(self)
    raw = _nomad_detector_raw(self)
    nframes = int(raw.shape[0]) if isinstance(raw, _h5py.Dataset) else 0
    _table, labels = _nomad_scanned_table(self)
    order = list(cfg.get("order", ["uu"]))
    n_components = max(1, len(order))
    counts = {}
    for p in order:
        counts[p] = len(range(_nomad_pol_offset(self, p), nframes, n_components)) if nframes else 0
    return {
        "is_nomad": _nomad_is_nomad(self),
        "mode": cfg.get("mode", "none"),
        "mode_label": cfg.get("mode_label", _NOMAD_MODE_LABELS.get(cfg.get("mode", "none"), "No unpacking / NR")),
        "order": order,
        "n_components": n_components,
        "source": cfg.get("source", "auto"),
        "user_confirmed": bool(cfg.get("user_confirmed", False)),
        "detected_components": cfg.get("detected_components", n_components),
        "detected_reason": cfg.get("detected_reason", ""),
        "raw_frames": nframes,
        "total_steps": _nomad_total_steps(self),
        "points_per_component": counts,
        "frames_divisible_by_components": (nframes % n_components == 0) if nframes else True,
        "scanned_labels": list(labels),
    }


def _nomad_variable(self, name, pol=None):
    table, _labels = _nomad_scanned_table(self)
    vec = None
    for key in _nomad_label_keys(name):
        vec = table.get(key)
    ...
    if vec is None:
        return None
    return _nomad_prepare_variable(self, vec, pol)


def _nomad_find_variable(self, aliases, pol=None):
    for label in aliases:
        vec = _nomad_variable(self, label, pol)
        if vec is not None:
            return vec, label
    return None, None


def _nomad_scanned_variables(self, pol=None):
    """Return all Nomad scanned variables, split/broadcast as used by pySAred."""
    _table, labels = _nomad_scanned_table(self)
    out = {}
    for label in labels:
        vec = _nomad_variable(self, label, pol)
...
def _nomad_monitor_source(self, pol=None):
    mon, label = _nomad_find_variable(self, _NOMAD_MONITOR_ALIASES, pol)
    if mon is not None:
        return mon, "monitor", label

    mon, label = _nomad_find_variable(self, _NOMAD_TOTALSUM_ALIASES, pol)
    if mon is not None:
        return mon, "totalsum_fallback", label

    mon, label = _nomad_find_variable(self, _NOMAD_ROISUM_ALIASES, pol)
    if mon is not None:
        return mon, "roisum_fallback", label

    return _np.ones(_nomad_npoints(self, pol), dtype=float), "ones_fallback", None


def _nomad_clean_slit_values(values):
    """Return slit values as clean Python-style floats without float32 display noise.

    Nomad slit widths are stored as float32 in current files, so values such as
    0.2 can appear as 0.20000000298023224 when converted directly to Python
    float.  Rounding to 8 decimals is still far beyond the physical precision
    needed here and keeps DB matching/display stable.
    """
    try:
        arr = _np.asarray(values, dtype=float).reshape(-1)
        if arr.size == 0:
            return arr
        arr = _np.round(arr, 8)
        arr[_np.isclose(arr, 0.0, atol=1e-12)] = 0.0
        return arr.astype(float, copy=False)
    except Exception:
        return values


def _nomad_numeric_series_from_dataset(self, rel_path, pol=None, clean_slits=False):
    """Read a scalar or vector Nomad dataset as a 1D series for this component.

    Scalars and single-element datasets are broadcast to the number of angular
    points.  If a future Nomad writer stores frame-indexed slit values, the same
    user-selected unpacking order/stride used for detector frames is applied.
    """
    ds = _nomad_dataset(self, rel_path)
    if not isinstance(ds, _h5py.Dataset):
        return None
    try:
        vec = _np.asarray(ds[()], dtype=float).reshape(-1)
    except Exception:
        return None
    if vec.size == 0:
        return None
    ...
    # Per-frame values should be unpacked exactly like detector frames and
    # scanned variables.  Constant/single-value slit datasets are broadcast.
    if isinstance(raw, _h5py.Dataset) and vec.size == int(raw.shape[0]):
        vec = _np.asarray(_nomad_prepare_variable(self, vec, pol), dtype=float).reshape(-1)
    elif vec.size == 1 and n > 1:
        vec = _np.full(n, float(vec[0]), dtype=float)

    if clean_slits:
        vec = _nomad_clean_slit_values(vec)
    return _np.asarray(vec, dtype=float).reshape(-1)


def _nomad_constant_series(self, rel_path, pol=None):
    # Kept for duration/preset compatibility; now also accepts vector datasets.
    return _nomad_numeric_series_from_dataset(self, rel_path, pol, clean_slits=False)


def _nomad_slit_series(self, rel_path, pol=None):
    return _nomad_numeric_series_from_dataset(self, rel_path, pol, clean_slits=True)


def _nomad_monitor_message(self):
    if not _nomad_is_nomad(self):
        return ""
    _mon, source, label = _nomad_monitor_source(self, None)
    if source == "monitor":
        return ""
    if source == "totalsum_fallback":
        return (
            "Warning: Monitor1 was not found in this Nomad .nxs file under "
            "/entry0/data_scan/scanned_variables; pySAred is using "
            f"'{label}' as the monitor fallback for the current reduction."
        )
    if source == "roisum_fallback":
        return (
            "Warning: Monitor1 and totalsum were not found in this Nomad .nxs file under "
            "/entry0/data_scan/scanned_variables; pySAred is using "
            f"'{label}' as a last-resort monitor fallback for the current reduction."
        )
    return (
        "Warning: Monitor1, totalsum, and roisum were not found in this Nomad .nxs file under "
        "/entry0/data_scan/scanned_variables; pySAred will use 1 as a temporary monitor."
    )


# Keep references to the already safety-patched HDF5 methods and add Nomad in front.
_h5resolver_instrument_name_prev = H5Resolver.instrument_name
_h5resolver_th_list_prev = H5Resolver.th_list
_h5resolver_tth_list_prev = H5Resolver.tth_list
_h5resolver_slit_lists_prev = H5Resolver.slit_lists
_h5resolver_pol_list_prev = H5Resolver.pol_list
_h5resolver_detector_stack_prev = H5Resolver.detector_stack
_h5resolver_roi_prev = H5Resolver.roi
_h5resolver_monitor_and_time_prev = H5Resolver.monitor_and_time
_h5resolver_lineouts_prev = H5Resolver.lineouts


def _instrument_name_nomad(self):
    if _nomad_is_nomad(self):
        name = _h5_text_value(_nomad_dataset(self, "SUPERADAM/name")).strip()
        return name or "SuperADAM"
    return _h5resolver_instrument_name_prev(self)


def _th_list_nomad(self):
    if _nomad_is_nomad(self):
        th, _label = _nomad_find_variable(self, _NOMAD_TH_ALIASES, "uu")
        return _safe_list_like(th)
    return _h5resolver_th_list_prev(self)


def _tth_list_nomad(self):
    if _nomad_is_nomad(self):
        v, _label = _nomad_find_variable(self, _NOMAD_TTH_ALIASES, "uu")
        if v is None:
            th, _label = _nomad_find_variable(self, _NOMAD_TH_ALIASES, "uu")
            if th is not None:
                v = 2 * _np.asarray(th, dtype=float)
        return _safe_list_like(v)
    return _h5resolver_tth_list_prev(self)


def _slit_lists_nomad(self):
    if _nomad_is_nomad(self):
        s1 = _nomad_slit_series(self, "SUPERADAM/VirtualSlit/s1h_actual_width", "uu")
        s2 = _nomad_slit_series(self, "SUPERADAM/VirtualSlit/s2h_actual_width", "uu")
        return s1, s2
    return _h5resolver_slit_lists_prev(self)


def _pol_list_nomad(self):
    if _nomad_is_nomad(self):
        return _nomad_component_order(self)
    return _h5resolver_pol_list_prev(self)


def _detector_stack_nomad(self, pol=None):
    if _nomad_is_nomad(self):
        raw = _nomad_detector_raw(self)
        if not isinstance(raw, _h5py.Dataset):
            return None
        n_components = _nomad_component_count(self)
        return _H5InterleavedDetectorView(
            raw,
            offset=_nomad_pol_offset(self, pol),
            stride=n_components if n_components > 1 else 1,
            transpose_yx=True,
        )
    return _h5resolver_detector_stack_prev(self, pol)


def _roi_nomad(self, images_hw=None):
    if _nomad_is_nomad(self):
        vals = []
        for rel in (
            "SUPERADAM/Detector1/roi_y0",
            "SUPERADAM/Detector1/roi_y1",
            "SUPERADAM/Detector1/roi_x0",
            "SUPERADAM/Detector1/roi_x1",
        ):
            val = _h5_scalar_value(_nomad_dataset(self, rel), None)
            if val is None:
                vals = []
                break
            vals.append(int(_np.ceil(float(val))))
        if len(vals) == 4:
            return vals
    return _h5resolver_roi_prev(self, images_hw)


def _monitor_and_time_nomad(self, pol=None):
    if _nomad_is_nomad(self):
        mon, _source, _label = _nomad_monitor_source(self, pol)
        mon = _np.asarray(mon, dtype=float)

        sec, _label = _nomad_find_variable(self, _NOMAD_TIME_ALIASES, pol)
        if sec is None:
            sec = _nomad_constant_series(self, "duration", pol)
        if sec is None:
            sec = _nomad_constant_series(self, "preset", pol)
        if sec is None:
            sec = _np.ones_like(mon, dtype=float)
        sec = _np.asarray(sec, dtype=float)
        return mon, sec
    return _h5resolver_monitor_and_time_prev(self, pol)


def _lineouts_nomad(self, pol=None):
    if _nomad_is_nomad(self):
        # No separate PONOS/OSS lineouts are written by the current Nomad file.
        return None
    return _h5resolver_lineouts_prev(self, pol)


H5Resolver.is_nomad_nxs = _nomad_is_nomad
H5Resolver.nomad_unpack_config = _nomad_unpack_config
H5Resolver.nomad_unpack_summary = _nomad_unpack_summary
H5Resolver.nomad_variable = _nomad_variable
H5Resolver.nomad_scanned_variables = _nomad_scanned_variables
H5Resolver.nomad_monitor_source = _nomad_monitor_source
H5Resolver.nomad_monitor_message = _nomad_monitor_message
H5Resolver.instrument_name = _instrument_name_nomad
H5Resolver.th_list = _th_list_nomad
H5Resolver.tth_list = _tth_list_nomad
H5Resolver.slit_lists = _slit_lists_nomad
H5Resolver.pol_list = _pol_list_nomad
H5Resolver.detector_stack = _detector_stack_nomad
H5Resolver.roi = _roi_nomad
H5Resolver.monitor_and_time = _monitor_and_time_nomad
H5Resolver.lineouts = _lineouts_nomad
# --- END: Nomad NeXus (.nxs) support for SuperADAM ---
...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.__create_element(self.label_h5Scans, [15, 5, 200, 20], "label_h5Scans", text=".h5 / .nxs files", font=font_headline, stylesheet="QLabel { color : blue; }")
...
class GUI(Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.SFM_export_pols = []  # Polarisation labels matching SFM_export_* buffers
    ...
    def _file_base_no_ext(self, path_or_name):
        return os.path.splitext(os.path.basename(str(path_or_name)))[0]

    def _format_number_compact(self, value, digits=8):
        """Compact numeric text for GUI fields/tables without changing calculations."""
        try:
            x = float(value)
            if not np.isfinite(x):
                return str(value)
            txt = f"{x:.{int(digits)}f}".rstrip("0").rstrip(".")
            return "0" if txt in ("", "-0") else txt
        except Exception:
            return str(value)

    def _slit_db_key(self, scan_name, s1hg, s2hg):
        """Stable key for matching DB scans by slit values, without float32 display noise."""
        return (
            str(scan_name) + ";" +
            self._format_number_compact(s1hg, digits=8) + ";" +
            self._format_number_compact(s2hg, digits=8)
        )

    def _warn_nomad_monitor_fallback(self, resolver):
        try:
            msg = resolver.nomad_monitor_message()
        except Exception:
            msg = ""
        if not msg:
            return
        try:
            ...
            pass
        try:
            shown = getattr(self, "_nomad_monitor_warning_shown", set())
            if msg not in shown:
                shown.add(msg)
                self._nomad_monitor_warning_shown = shown
                print(msg)
                QtWidgets.QMessageBox.information(self, "Nomad monitor data", msg)
        except Exception:
            pass


    def _configure_nomad_file(self, file_path, force=False):
        """Ask the user how a Nomad .nxs file should be unpacked and store the answer."""
        try:
            if not str(file_path).lower().endswith(".nxs"):
                return True
            key = _nomad_file_key_from_path(file_path)
            if force:
                _NOMAD_UNPACK_CONFIG_BY_FILE.pop(key, None)
            elif _nomad_get_unpack_config_for_path(file_path) is not None:
                return True

            with h5py.File(file_path, "r") as f:
                scan = f[list(f.keys())[0]]
                resolver = H5Resolver(scan)
                if not resolver.is_nomad_nxs():
                    return True
                info = resolver.nomad_unpack_summary()

            cfg = self._nomad_unpacking_dialog(file_path, info)
            if cfg is None:
                try:
                    self.statusbar.showMessage("Nomad import cancelled: no unpacking mode was selected.")
                except Exception:
                    pass
                return False
            _nomad_set_unpack_config_for_path(file_path, cfg)
            return True
        except Exception as ex:
            try:
                self.statusbar.showMessage(f"Error while reading Nomad file '{os.path.basename(str(file_path))}': {ex}")
            except Exception:
                pass
            return False

    def _nomad_unpacking_dialog(self, file_path, info):
        """Small import dialog: auto-detected mode plus user-confirmed/dragged order."""
        detected = int(info.get("detected_components", 1) or 1)
        detected_mode = _nomad_mode_from_n_components(detected)
        raw_frames = int(info.get("raw_frames", 0) or 0)
        total_steps = int(info.get("total_steps", 0) or 0)
        labels = info.get("scanned_labels", []) or []

        dialog = QtWidgets.QDialog(self)
        dialog.setWindowTitle("Nomad detector / spin-state unpacking")

        # Use a readable import dialog while keeping it usable on small screens.
        screen = QtWidgets.QApplication.primaryScreen()
        if screen is not None:
            available = screen.availableGeometry()
            dialog_w = max(560, min(720, int(available.width() * 0.72)))
            dialog_h = max(360, min(560, int(available.height() * 0.82)))
            min_w = max(500, min(600, int(available.width() * 0.62)))
            min_h = max(320, min(420, int(available.height() * 0.58)))
        else:
            dialog_w, dialog_h, min_w, min_h = 640, 500, 560, 360
        dialog.resize(dialog_w, dialog_h)
        dialog.setMinimumSize(QtCore.QSize(min_w, min_h))

        base_font = QtGui.QFont(dialog.font())
        current_pt = base_font.pointSize() if base_font.pointSize() > 0 else 10
        target_pt = 10 if platform.system() == 'Windows' else 11
        base_font.setPointSize(max(current_pt, target_pt))
        ...
        header = QtWidgets.QLabel(
            "pySAred detected a Nomad .nxs file. Confirm how the detector frames and "
            "frame-indexed scanned variables should be unpacked."
        )
        ...
        detail = QtWidgets.QLabel(
            f"File: {os.path.basename(str(file_path))}\n"
            f"Raw detector frames: {raw_frames}\n"
            f"Nomad total_steps: {total_steps if total_steps else 'not found'}\n"
            f"Suggested mode: {_NOMAD_MODE_LABELS.get(detected_mode, detected_mode)}\n"
            f"Reason: {info.get('detected_reason', '')}\n"
            f"Scanned variables: {', '.join(labels) if labels else 'not found'}"
        )
        detail.setWordWrap(True)
        detail.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        detail.setStyleSheet("QLabel { background: #f6f6f6; border: 1px solid #cccccc; padding: 6px; }")
        ...
        mode_combo.setFont(base_font)
        mode_combo.setMinimumHeight(28)
        mode_combo.addItem("No unpacking / NR - use frames exactly as stored", "none")
        mode_combo.addItem("2-state NSF - split as I+ and I- (pySAred labels uu, dd)", "nsf_2")
        mode_combo.addItem("4-state PNR - split into uu, ud, dd, du", "pnr_4")
        for row in range(mode_combo.count()):
            if mode_combo.itemData(row) == detected_mode:
                mode_combo.setCurrentIndex(row)
                break
        form.addRow("Mode:", mode_combo)
        layout.addLayout(form)

        order_label = QtWidgets.QLabel("Frame order within each angular point. Drag rows, or use Move up/down, to match the acquisition order:")
        order_label.setWordWrap(True)
        layout.addWidget(order_label)

        order_list = QtWidgets.QListWidget(dialog)
        order_list.setFont(base_font)
        order_list.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        order_list.setDefaultDropAction(QtCore.Qt.MoveAction)
        order_list.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        order_list.setUniformItemSizes(False)
        order_list.setMinimumHeight(86)
        order_list.setMaximumHeight(150)
        layout.addWidget(order_list)

        button_row = QtWidgets.QHBoxLayout()
        up_btn = QtWidgets.QPushButton("Move up", dialog)
        down_btn = QtWidgets.QPushButton("Move down", dialog)
        up_btn.setMinimumHeight(28)
        down_btn.setMinimumHeight(28)
        ...
        warning_label.setStyleSheet("QLabel { color : #a00000; }")
        layout.addWidget(warning_label)

        def _current_mode():
            return str(mode_combo.currentData())

        def _current_order():
            return [order_list.item(i).text() for i in range(order_list.count())]

        def _set_order_for_mode(mode):
            order_list.clear()
            item_h = max(26, base_font.pointSize() + 14)
            for label in _nomad_default_order_for_mode(mode):
                item = QtWidgets.QListWidgetItem(label)
                item.setFont(base_font)
                item.setSizeHint(QtCore.QSize(100, item_h))
                order_list.addItem(item)
            enabled = len(_nomad_default_order_for_mode(mode)) > 1
            order_list.setEnabled(enabled)
            up_btn.setEnabled(enabled)
            down_btn.setEnabled(enabled)
            _update_warning()

        def _move_selected(delta):
            row = order_list.currentRow()
            if row < 0:
                return
            new_row = row + int(delta)
            if new_row < 0 or new_row >= order_list.count():
                return
            item = order_list.takeItem(row)
            order_list.insertItem(new_row, item)
            order_list.setCurrentRow(new_row)
            _update_warning()

        def _update_warning():
            order = _current_order()
            ncomp = max(1, len(order))
            msg = ""
            if raw_frames and ncomp > 1 and raw_frames % ncomp != 0:
                msg = (
                    f"Warning: {raw_frames} detector frames are not divisible by {ncomp} components. "
                    "The last angular point would be incomplete for at least one component."
                )
            elif _current_mode() == "nsf_2":
                msg = "NSF convention: pySAred label uu is I+ (= uu + ud), and dd is I- (= dd + du)."
            elif _current_mode() == "pnr_4":
                msg = "PNR convention: the labels above are used as pySAred channels and export tags."
            warning_label.setText(msg)

        mode_combo.currentIndexChanged.connect(lambda _idx: _set_order_for_mode(_current_mode()))
        up_btn.clicked.connect(lambda: _move_selected(-1))
        down_btn.clicked.connect(lambda: _move_selected(1))
        try:
            order_list.model().rowsMoved.connect(lambda *_args: _update_warning())
        except Exception:
            pass
        _set_order_for_mode(_current_mode())
        ...
        def _accept():
            mode = _current_mode()
            order = _current_order()
            cfg = _nomad_normalize_unpack_config({
                "mode": mode,
                "order": order,
                "source": "user",
                "user_confirmed": True,
                "detected_components": detected,
                "detected_reason": info.get("detected_reason", ""),
            }, detected, info.get("detected_reason", ""))

            ncomp = int(cfg.get("n_components", 1) or 1)
            if raw_frames and ncomp > 1 and raw_frames % ncomp != 0:
                reply = QtWidgets.QMessageBox.question(
                    dialog,
                    "Incomplete component group",
                    f"{raw_frames} detector frames are not divisible by {ncomp} selected components. Continue anyway?",
                    QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                    QtWidgets.QMessageBox.No,
                )
                if reply != QtWidgets.QMessageBox.Yes:
                    return
            dialog._nomad_unpack_config = cfg
            dialog.accept()
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            return getattr(dialog, "_nomad_unpack_config", None)
        return None

    def _series_summary(self, name, values):
        try:
            ...
            if np.nanmax(arr) == np.nanmin(arr):
                return f"{name}: {self._format_number_compact(arr[0])} (constant, n={arr.size})"
            return (
                f"{name}: n={arr.size}, "
                f"min={self._format_number_compact(np.nanmin(arr))}, "
                f"max={self._format_number_compact(np.nanmax(arr))}, "
                f"first={self._format_number_compact(arr[0])}, "
                f"last={self._format_number_compact(arr[-1])}"
            )
        except Exception:
            return f"{name}: unavailable"

    def _format_skip_summary(self, skipped):
        skipped = list(skipped or [])
        if not skipped:
            return "none"
        counts = {}
        for _idx, reason in skipped:
            counts[reason] = counts.get(reason, 0) + 1
        parts = [f"{reason}: {count}" for reason, count in sorted(counts.items())]
        sample = ", ".join([f"{idx} ({reason})" for idx, reason in skipped[:40]])
        if len(skipped) > 40:
            sample += f", ... +{len(skipped) - 40} more"
        return "; ".join(parts) + f". Indices: {sample}"

    def _nomad_summary_lines(self, resolver):
        lines = []
        try:
            if not resolver.is_nomad_nxs():
                lines.append("Nomad unpacking: not applicable")
                return lines
            info = resolver.nomad_unpack_summary()
            lines.append("Nomad unpacking:")
            lines.append(f"  Mode: {info.get('mode_label', '')}")
            lines.append(f"  User confirmed: {info.get('user_confirmed', False)}")
            lines.append(f"  Component order: {', '.join(info.get('order', []))}")
            lines.append(f"  Raw detector frames: {info.get('raw_frames', 0)}")
            lines.append(f"  Nomad total_steps: {info.get('total_steps', 0) or 'not found'}")
            lines.append(f"  Detected components: {info.get('detected_components', '')}")
            lines.append(f"  Detection reason: {info.get('detected_reason', '')}")
            counts = info.get("points_per_component", {}) or {}
            lines.append("  Points per component: " + ", ".join([f"{k}={v}" for k, v in counts.items()]))
            lines.append(f"  Frame count divisible by selected components: {info.get('frames_divisible_by_components', True)}")
            labels = info.get("scanned_labels", []) or []
            lines.append("  Scanned variables unpacked with same mode: " + (", ".join(labels) if labels else "none found"))
            try:
                _mon, source, label = resolver.nomad_monitor_source(None)
                lines.append(f"  Monitor source: {label if label else source} ({source})")
            except Exception:
                pass
        except Exception as ex:
            lines.append(f"Nomad unpacking: summary unavailable ({ex})")
        return lines

    def _write_reduction_summary(self, summary_path, input_file, reduction_mode, output_files, pol_summaries,
                                 resolver=None, roi=None, bkg_roi=None, s1hg_list=None, s2hg_list=None):
        try:
            lines = []
            lines.append("pySAred Data reduction Summary")
            lines.append("Generated: " + datetime.now().isoformat(timespec="seconds"))
            lines.append("Input file: " + str(input_file))
            lines.append("Reduction mode: " + str(reduction_mode))
            lines.append("")

            if resolver is not None:
                lines.extend(self._nomad_summary_lines(resolver))
                lines.append("")

            lines.append("Output data files:")
            ...
            lines.append("Instrument and export values:")
            lines.append(f"  Wavelength: {self.lineEdit_instrument_wavelength.text()}")
            lines.append(f"  Wavelength resolution: {self.lineEdit_instrument_wavelengthResolution.text()}")
            lines.append(f"  Distance S1-sample: {self.lineEdit_instrument_distanceS1ToSample.text()}")
            lines.append(f"  Distance S2-sample: {self.lineEdit_instrument_distanceS2ToSample.text()}")
            lines.append(f"  Distance sample-detector: {self.lineEdit_instrument_distanceSampleToDetector.text()}")
            lines.append(f"  Full angular offset: {self.lineEdit_instrument_offsetFull.text()}")
            lines.append(f"  Export angle: {self.comboBox_export_angle.currentText()}")
            lines.append(f"  Add resolution column: {bool(self.checkBox_export_addResolutionColumn.isChecked())}")
            lines.append(f"  Original Sared resolution mode: {bool(self.checkBox_export_resolutionLikeSared.isChecked())}")
            lines.append(f"  Remove zero intensities: {bool(self.checkBox_export_removeZeros.isChecked())}")
            lines.append("")

            lines.append("Corrections:")
            lines.append(f"  Divide by monitor/time: {bool(self.checkBox_reductions_divideByMonitorOrTime.isChecked())}")
            if self.checkBox_reductions_divideByMonitorOrTime.isChecked():
                lines.append(f"  Normalization channel: {self.comboBox_reductions_divideByMonitorOrTime.currentText()}")
            lines.append(f"  Background subtraction: {bool(self.checkBox_reductions_subtractBkg.isChecked())}")
            lines.append(f"  Background skip threshold: {self.lineEdit_reductions_subtractBkg_Skip.text() or '0'}")
            lines.append(f"  Overillumination correction: {bool(self.checkBox_reductions_overilluminationCorr.isChecked())}")
            lines.append(f"  Sample shape: {self.comboBox_sampleShape.currentText()}")
            lines.append(f"  Sample length: {self.lineEdit_sampleLen.text()}")
            try:
                lines.append(f"  Sample transverse size: {self.lineEdit_sampleDy.text()}")
            except Exception:
                pass
            lines.append(f"  Sample curvature: {self.lineEdit_instrument_sampleCurvature.text()}")
            lines.append(f"  Direct-beam normalization: {bool(self.checkBox_reductions_normalizeByDB.isChecked())}")
            lines.append(f"  Direct-beam attenuator correction: {self.lineEdit_reductions_attenuatorDB.text() if self.checkBox_reductions_attenuatorDB.isChecked() else 'not applied'}")
            lines.append(f"  Scale factor: {self.lineEdit_reductions_scaleFactor.text() if self.checkBox_reductions_scaleFactor.isChecked() else 'not applied'}")
            lines.append("")

            lines.append("ROI settings:")
            if roi is not None:
                try:
                    y0, y1, x0, x1 = roi
                    lines.append(f"  Main ROI: y_top={y0}, y_bottom={y1}, x_left={x0}, x_right={x1}")
                except Exception:
                    lines.append(f"  Main ROI: {roi}")
            if bkg_roi is not None:
                lines.append(f"  Background ROI: {bkg_roi}")
            elif self.checkBox_reductions_subtractBkg.isChecked():
                lines.append("  Background ROI: generated from the main ROI during reduction")
            lines.append("")

            lines.append("Slit values:")
            lines.append("  " + self._series_summary("s1", s1hg_list))
            lines.append("  " + self._series_summary("s2", s2hg_list))
            lines.append("")

            lines.append("Skipped/exported points:")
            for item in pol_summaries or []:
                lines.append(f"  Component {item.get('pol', '')}:")
                lines.append(f"    Total candidate points: {item.get('total', '')}")
                lines.append(f"    Exported points: {item.get('written', '')}")
                lines.append(f"    Skipped points: {self._format_skip_summary(item.get('skipped', []))}")
            ...
            try:
                self.statusbar.showMessage(f"Warning: could not write data reduction summary: {ex}")
            except Exception:
                pass
            return False
    ##--> menu options
    ...
    def f_button_importRemoveScans(self):
        ...
        if self.sender().objectName() == "pushButton_importScans":
            ...
            files_import = QtWidgets.QFileDialog().getOpenFileNames(None, "FileNames", self.dir_current, "Data files (*.h5 *.nxs);;HDF5 files (*.h5);;NeXus files (*.nxs)")
            if files_import[0] == []: return

            files_to_add = []
            for FILE in files_import[0]:
                if self._configure_nomad_file(FILE, force=True):
                    files_to_add.append(FILE)
            if files_to_add == []: return

            # Next "Import scans" will open last dir
            self.dir_current = files_to_add[0][:files_to_add[0].rfind("/")]
    ...
    def f_button_importRemoveDB(self):
        ...
        if self.sender().objectName() == "pushButton_importDB":
            ...
            for i in range(self.tableWidget_DB.rowCount()-1, -1, -1):
                files_to_add.append(self.tableWidget_DB.item(i, 1).text())
    ...
    def f_button_reduceAll(self):
        ...
        for i in range(0, self.tableWidget_scans.rowCount()):
            ...
            with h5py.File(file_full, 'r') as FILE:
                ...
                for p in pols:
                    ...
                    with open(out_path, "w") as new_file:
                        for idx, th in enumerate(th_list):
                            if idx >= scan_intens.shape[0]:
                                skipped_points.append((idx, "missing detector frame for this component"))
                                continue
                            ...
                            if Intens == 0 and self.checkBox_export_removeZeros.isChecked():
                                skipped_points.append((idx, "zero raw ROI intensity removed"))
                                continue
                            ...
                            if self.checkBox_reductions_normalizeByDB.isChecked():
                                try:
                                    db_key = self._slit_db_key(FILE_DB, s1hg, s2hg)
                            ...
                            if idx == 0 and getattr(self, 'export_skip_first_point', True):
                                skipped_points.append((idx, "first point skipped by export setting"))
                                continue
                            if Intens == 0 and self.checkBox_export_removeZeros.isChecked():
                                skipped_points.append((idx, "zero corrected intensity removed"))
                                continue
                            if not (np.isfinite(angle) and np.isfinite(Intens) and np.isfinite(IntensErr) and np.isfinite(Resolution)):
                                skipped_points.append((idx, "non-finite output value"))
                                continue
                ...
                summary_path = os.path.join(dir_saveFile, file_name + "_Data_reduction_summary.txt")
                self._write_reduction_summary(
                    summary_path, file_full, "Batch reduction", output_files, pol_summaries,
                    resolver=R, roi=(y0, y1, x0, x1), bkg_roi=None,
                    s1hg_list=s1hg_list, s2hg_list=s2hg_list,
                )
    ...
    def f_button_reduceSFM(self):
        ...
        detector = getattr(self, "SFM_export_pols", []) or ["uu", "du", "ud", "dd"]
        base_name = self._file_base_no_ext(self.SFM_FILE)
        output_files = []
        pol_summaries = []
        ...
        for i in range(0, len(self.SFM_export_Qz)):
            ...
            pol_tag = detector[i] if i < len(detector) else str(i)
            out_path = dir_saveFile + base_name + "_" + str(pol_tag) + " (" + SFM_DB_file_export + ")" + " SFM.dat"
            skipped_points = list(getattr(self, "SFM_preview_skipped", [[]] * len(self.SFM_export_Qz))[i]) if i < len(getattr(self, "SFM_preview_skipped", [])) else []
            export_indices = getattr(self, "SFM_export_indices", [])
            export_indices_i = export_indices[i] if i < len(export_indices) else []
            written_points = 0

            with open(out_path, "w") as new_file:
                for j in range(0, len(self.SFM_export_Qz[i])):
                    original_idx = export_indices_i[j] if j < len(export_indices_i) else j
                    if self.SFM_export_I[i][j] == 0 and self.checkBox_export_removeZeros.isChecked():
                        skipped_points.append((original_idx, "zero intensity removed at SFM export"))
                        continue
            ...
            pol_summaries.append({
                "pol": pol_tag,
                "total": getattr(self, "SFM_total_candidate_points", len(self.SFM_export_Qz[i])),
                "written": written_points,
                "skipped": skipped_points,
            })

        # Write a separate summary file; the exported data files themselves stay unchanged.
        if self.SFM_FILE and self._configure_nomad_file(self.SFM_FILE):
            try:
                with h5py.File(self.SFM_FILE, 'r') as FILE:
                    ...
                    roi = (
                        ...
                        int(self.lineEdit_SFM_detectorImage_roiY_bottom.text()),
                    ...
                    summary_path = os.path.join(dir_saveFile, base_name + "_SFM_Data_reduction_summary.txt")
                    self._write_reduction_summary(
                        summary_path, self.SFM_FILE, "Single File Mode reduction", output_files, pol_summaries,
                        resolver=R, roi=roi, bkg_roi=bkg_roi,
                        s1hg_list=s1hg_list, s2hg_list=s2hg_list,
                    )
            except Exception as ex:
                try:
                    self.statusbar.showMessage(f"Warning: SFM summary was not written: {ex}")
                except Exception:
                    pass
    ...
    def f_overilluminationCorrCoeff(self, s1hg, s2hg, th):
        ...
        if sample_len is None:
            return [1, s2hg]
        config = self._format_number_compact(s1hg, digits=8) + " " + self._format_number_compact(s2hg, digits=8) + " " + str(th) + " " + str(sample_len) + " " + str(shape_mode) + " " + str(sample_dy) + " " + self.lineEdit_instrument_distanceS1ToSample.text() + " " + self.lineEdit_instrument_distanceS2ToSample.text()
    ...
    def f_DB_analaze(self):
        ...
        for i in range(0, self.tableWidget_DB.rowCount()):
            db_path = self.tableWidget_DB.item(i,1).text()
            if not self._configure_nomad_file(db_path):
                continue
            with h5py.File(db_path, 'r') as FILE_DB:
                ...
                for j in range(0, len(th_list)):
                    ...
                    scan_slitsMonitor = self._slit_db_key(self.tableWidget_DB.item(i, 0).text()[:5], s1hg_list[j], s2hg_list[j])
    ...
    def f_SFM_detectorImage_load(self):
        ...
        if not self._configure_nomad_file(self.SFM_FILE):
            return
        ...
        try:
            ...
            with h5py.File(self.SFM_FILE, 'r') as FILE:
                ...
                for i in range(N):
                    ...
                    s1_val = "" if s1_list is None or len(s1_list) <= i else self._format_number_compact(s1_list[i])
                    s2_val = "" if s2_list is None or len(s2_list) <= i else self._format_number_compact(s2_list[i])
    ...
    def f_SFM_monitors_export(self):
        ...
        try:
            ...
            dir_saveFile = self.lineEdit_saveAt.text() if self.lineEdit_saveAt.text() else self.dir_current
            base = self._file_base_no_ext(self.comboBox_SFM_scan.currentText())
    ...
    def f_SFM_detectorImage_draw(self):
        ...
        with h5py.File(self.SFM_FILE, 'r') as FILE:
            ...
            if idx >= stack.shape[0]:
                self.statusbar.showMessage("Selected angular point has no detector frame for this component.")
                return

            self.lineEdit_SFM_detectorImage_slits_s1hg.setText(self._format_number_compact(s1hg_list[idx]) if s1hg_list is not None and len(s1hg_list) > idx else "")
            self.lineEdit_SFM_detectorImage_slits_s2hg.setText(self._format_number_compact(s2hg_list[idx]) if s2hg_list is not None and len(s2hg_list) > idx else "")
    ...
    def f_SFM_reflectivityPreview_load(self):
        ...
        self.SFM_export_pols = []
        self.SFM_export_indices = []
        self.SFM_preview_skipped = []
        self.SFM_total_candidate_points = 0
        ...
        with h5py.File(self.SFM_FILE, 'r') as FILE:
            ...
            # now compute/refplot per channel (same math as original).  Use R.pol_list()
            # so SFM export order follows the user-confirmed Nomad component order.
            pol_data_map = {"uu": self.SFM_psdUU, "du": self.SFM_psdDU, "ud": self.SFM_psdUD, "dd": self.SFM_psdDD}
            pol_color_map = {"uu": [0, 0, 0], "du": [0, 0, 255], "ud": [0, 255, 0], "dd": [255, 0, 0]}
            for colorIndex, p in enumerate(pols):
                data = pol_data_map.get(p, [])
                if (data is None) or (hasattr(data, "size") and data.size == 0) or (hasattr(data, "__len__") and len(data) == 0):
                    continue

                color, monitorData = pol_color_map.get(p, [0, 0, 0]), first_non_none(mon_p.get(p), mon0)
                ...
                SFM_export_indices_onePol, SFM_skipped_onePol = [], []
                self.SFM_total_candidate_points = len(th_list)

                for i, th in enumerate(th_list):
                    if i >= data.shape[0]:
                        SFM_skipped_onePol.append((i, "missing detector frame for this component"))
                        continue
                    th = th - float(self.lineEdit_instrument_offsetFull.text())
                    Qz = (4 * np.pi / float(self.lineEdit_instrument_wavelength.text())) * np.sin(np.radians(th))
                    s1hg, s2hg = (s1hg_list[i] if s1hg_list is not None else 0), (s2hg_list[i] if s2hg_list is not None else 0)
                    monitor = monitorData[i] if (monitorData is not None and len(monitorData) > i) else 1
                    ...
                    if self.checkBox_reductions_normalizeByDB.isChecked():
                        try:
                            db_key = self._slit_db_key(self.SFM_DB_FILE, s1hg, s2hg)
                            DB_intens = float(self.DB_INFO[db_key].split(";")[0]) * self.DB_attenFactor
                            DB_err = float(self.DB_INFO[db_key].split(";")[1]) * self.DB_attenFactor
                    ...
                    if not (np.isfinite(Intens) and np.isfinite(IntensErr) and np.isfinite(Qz) and np.isfinite(Resolution)):
                        SFM_skipped_onePol.append((i, "non-finite preview value"))
                        continue
                    ...
                    if Intens >= 0 and i < show_last and i > show_first:
                        ...
                        SFM_export_indices_onePol.append(i)
                        ...
                        if Intens < 0:
                            SFM_skipped_onePol.append((i, "negative intensity excluded from SFM preview/export"))
                        elif i <= show_first:
                            SFM_skipped_onePol.append((i, "left skip-points setting"))
                        else:
                            SFM_skipped_onePol.append((i, "right skip-points setting"))

                # export buffers per pol
                self.SFM_export_pols.append(p)
                ...
                self.SFM_export_indices.append(SFM_export_indices_onePol)
                self.SFM_preview_skipped.append(SFM_skipped_onePol)
    ...
    def f_SFM_2Dmap_export(self):
        ...
        if self.comboBox_SFM_2Dmap_axes.currentText() == "Pixel vs. Point":
            with open(dir_saveFile + self._file_base_no_ext(self.SFM_FILE) + "_" + self.comboBox_SFM_2Dmap_polarisation.currentText() + " 2Dmap(Pixel vs. Point).dat", "w") as newFile_2Dmap:
            ...
            # Matrix
            with open(dir_saveFile + self._file_base_no_ext(self.SFM_FILE) + "_" + self.comboBox_SFM_2Dmap_polarisation.currentText() + " 2Dmap_(Alpha_i vs. Alpha_f)).dat", "w") as newFile_2Dmap_aif:
                # header
            ...
            # Points (full)
            with open(dir_saveFile + self._file_base_no_ext(self.SFM_FILE) + "_" + self.comboBox_SFM_2Dmap_polarisation.currentText() + " 2Dmap_(Alpha_i vs. Alpha_f))_Points.dat", "w") as newFile_2Dmap_aifPoints:
            ...
            with open(dir_saveFile + self._file_base_no_ext(self.SFM_FILE) + "_" + self.comboBox_SFM_2Dmap_polarisation.currentText() + " points_(Qx, Qz, intens).dat", "w") as newFile_2Dmap_Qxz:
